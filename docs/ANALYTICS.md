...
| `tool_option` | A control is changed — once per control per page load | `option` (control id such as `opt-sort`, `mode`, `preset`, `swap`), `value` (the select's visible label, `true`/`false` for a checkbox, the preset id, or `(text)` for a free-text box). Also fired outside tool pages: `favorite` (`add`/`remove`, card stars and the command palette), and on the home page `home_category` (category slug or `all`) and `home_filter` (`(text)`); `settings_reset` when a visitor resets a tool's remembered settings; `empty_cta` (`paste`/`sample`) when a button inside an empty input box is used (it then presses the tool's own Paste or sample control, which report as usual). Restoring remembered settings does not fire `tool_option` (tracking is muted during the restore) | Which options and presets earn their place in the UI |
