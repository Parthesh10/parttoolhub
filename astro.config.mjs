...
import { publishedGuideFiles } from './src/data/guide-files.ts';
...
// Guides the same way: updatedOn when the content changed after publishing, else publishedOn.
const guides = publishedGuideFiles();
for (const g of guides) lastmodByPath.set(guidePath(g.slug), new Date(`${g.updatedOn || g.publishedOn}T00:00:00Z`));

export default defineConfig({
  site: SITE.url,
  trailingSlash: 'never',
  build: { format: 'file' },
  // Guides render code as plain <pre><code>, like the tool pages; no highlighter to ship or theme.
  ...
  integrations: [
    sitemap({
      // The /guides hub is noindex while no guide is published, so it stays out until one is.
      filter: (page) => !page.includes('/404') && (guides.length > 0 || new URL(page).pathname !== GUIDES_HUB),
      serialize: (item) => {
