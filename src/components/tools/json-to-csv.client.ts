...
function render() {
  ...
  const result = jsonToCsv(raw, { delimiter: delimiterValue });
  trackRun(
    'convert',
    result.ok,
    result.ok
      ? undefined
      : /same shape/.test(result.error) ? 'mixed_shape'
        : /top level must be/.test(result.error) ? 'not_array'
          : 'json',
  );

  if (result.ok) {
    output.value = result.output;
    outputStat.textContent = `${plural(result.count, 'row')} · ${plural(result.output.length, 'character')} · ${formatBytes(result.output)}`;
    status.hidden = true;
  } else {
    output.value = '';
...
delimiter.addEventListener('input', render);
delimiter.addEventListener('change', () => trackOption(delimiter));
