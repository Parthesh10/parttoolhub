...
function render() {
  ...
  outputStat.textContent = result.length
    ? `${plural(result.split('\n').length, 'line')} · ${plural(result.length, 'character')} · ${formatBytes(result)}`
    : plural(result.length, 'character');
  if (raw.trim()) trackRun(mode.value === 'title' ? `title:${style.value}` : mode.value, true);
}
...
for (const c of [mode, style, preserve]) {
